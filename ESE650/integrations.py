...
from neural_mmo.ESE650.deposit_env import ForageEnv
...
def rllib_env_cls():
    ...
    class RLlibEnv(ForageEnv, rllib.MultiAgentEnv):
...
class CleanRLForageEnv(SB3ForageEnv):
    def __init__(self, config):
        super().__init__(config)
...
def cleanrl_vec_envs(config_cls, eval_config_cls=None, verbose=True):
    ...
    config    = config_cls()
    dummy_env = CleanRLForageEnv(config)
