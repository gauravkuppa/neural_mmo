...
import neural_mmo.baselines.config as base_config
from neural_mmo.baselines.config import scale
import config as ForageConfig
...
def run_tune_experiment(config, trainer_wrapper, rllib_env=wrapper.RLlibForageEnv):
    ...
    rllib.models.ModelCatalog.register_custom_model(
        'godsword', wrapper.RLlibForagePolicy)
...
class CLI():
    ...
    def __init__(self, **kwargs):
        ...
        config = 'ForageConfigDebug'
        if 'config' in kwargs:
            config = kwargs.pop('config')

        config = attrgetter(config)(ForageConfig)()
